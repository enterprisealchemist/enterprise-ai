...
const PresentationSkeleton: React.FC<PresentationSkeletonProps> = ({ slides, currentSlide, setCurrentSlide }) => {
  ...
  return (
    <div className="flex flex-col items-center justify-center h-screen bg-white-off p-4">
      <div className="bg-white rounded-lg shadow-lg p-6 w-full max-w-6xl aspect-video flex flex-col">
        <h1 className="text-2xl md:text-3xl font-bold mb-4 text-rich-black">Artificial Intelligence: Beyond an Upgrade</h1>
        <div className="flex-grow overflow-auto mb-4">
          <CurrentSlide {...slides[currentSlide].props} />
        </div>
        <div className="flex justify-between items-center mt-auto">
          <button onClick={prevSlide} className="bg-lapis-lazuli text-white-off p-2 rounded-full hover:bg-paynes-gray transition-colors">
            <ChevronLeft size={24} />
          </button>
          <span className="text-lg text-space-cadet">{currentSlide + 1} / {slides.length}</span>
          <button onClick={nextSlide} className="bg-lapis-lazuli text-white-off p-2 rounded-full hover:bg-paynes-gray transition-colors">
